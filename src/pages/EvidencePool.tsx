        ...
        <div className="divide-y divide-slate-200 border-b border-slate-200 bg-white">
          {visible.map((card) => (
            <article key={card.id} className="px-4 py-4">
              ...
              {expanded === card.id ? <div className="mt-3 border-l-2 border-blue-200 pl-4 text-xs text-slate-600"><div className="grid gap-3 sm:grid-cols-3"><div><span className="font-semibold text-slate-700">飞书记录</span><div className="mt-1">verified_user_feedback · {(evidenceIdsByCard.get(card.id) ?? []).join(" / ")}</div></div><div><span className="font-semibold text-slate-700">痛点引用</span><div className="mt-1">{card.relatedPainPoints.join(" / ")}</div></div><div><span className="font-semibold text-slate-700">页面引用</span><div className="mt-1 flex items-center gap-1"><Link2 size={12} />{card.relatedPages.join(" / ")}</div></div></div><div className="mt-3 border-t border-slate-100 pt-3"><div className="font-semibold text-slate-700">已审核公开来源</div><div className="mt-2 flex flex-wrap gap-2">{(evidenceIdsByCard.get(card.id) ?? []).map((id) => { const item = feedbackById.get(id); return item?.sourceUrl ? <a key={id} href={item.sourceUrl} target="_blank" rel="noreferrer" className="inline-flex items-center gap-1 rounded border border-slate-200 bg-white px-2 py-1 font-semibold text-blue-700 hover:border-blue-300"><ExternalLink size={11} />{id} · {item.source}</a> : <span key={id}>{id}</span>; })}</div></div></div> : null}
            </article>
          ))}
        </div>
      </section>
