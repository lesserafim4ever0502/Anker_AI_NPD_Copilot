    ...
    <div className="space-y-6">
      ...
      {tab === "Scope" ? <>
        <section className="grid grid-cols-1 gap-5 xl:grid-cols-[minmax(0,1fr)_360px]">
          ...
          <aside className="panel-muted"><div className="section-kicker"><Shield size={14} /> 非目标</div><ul className="mt-3 space-y-3 text-sm leading-6 text-slate-700">{proposalPrd.outOfScope.map((item) => <li key={item} className="border-l-2 border-slate-300 pl-3">{item}</li>)}</ul></aside>
        </section>
        <section><div className="mb-3 flex items-end justify-between"><div><h3 className="section-title">P0 规则 MVP</h3><p className="section-subtitle">有限拓扑、可追溯、允许返回未知</p></div><span className="text-xs text-slate-500">{p0.length} 项 P0 功能</span></div><div className="grid grid-cols-1 gap-px overflow-hidden border border-slate-200 bg-slate-200 md:grid-cols-2">{p0.map((feature, index) => <article key={feature.feature} className="bg-white p-4"><div className="flex items-start gap-3"><span className="flex h-6 w-6 shrink-0 items-center justify-center rounded bg-blue-50 text-xs font-semibold text-blue-700">{index + 1}</span><div><h4 className="font-semibold text-ink">{feature.feature}</h4><p className="mt-2 text-sm leading-6 text-slate-600">{feature.value}</p><div className="mt-2 text-xs text-slate-400">{feature.relatedPainPoint}</div></div></div></article>)}</div></section>
      </> : null}
