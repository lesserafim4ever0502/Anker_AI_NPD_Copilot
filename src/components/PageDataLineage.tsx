  ...
  return (
    <section className="lineage-strip" aria-label={`${lineage.stage}数据血缘`}>
      <div className="lineage-stage">
        <Database size={15} />
        <div><div className="text-[10px] uppercase text-slate-400">飞书数据血缘</div><div className="text-xs font-semibold text-slate-700">{lineage.stage}</div></div>
      </div>
      ...
      <div className="hidden min-w-0 lg:block"><div className="text-[10px] uppercase text-slate-400">处理规则</div><div className="truncate text-xs text-slate-600">{lineage.process}</div></div>
      <div className="lineage-output" title={`${activeSnapshot.source} · ${activeSnapshot.version}`}><FileCheck2 size={14} /><span>{hasSnapshotData ? lineage.output : "快照未载入"}</span></div>
    </section>
  );
}
