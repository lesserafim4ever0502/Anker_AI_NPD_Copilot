  ...
  return (
    <div className="space-y-6">
      ...
      <section className="run-progress-panel">
        <div className="flex items-end justify-between gap-4"><div><h3 className="section-title">当前 Run 进度</h3><p className="section-subtitle">{run.name} · {run.currentStage}</p></div><span className="text-xs text-slate-500">已完成 {run.stageProgress.filter((stage) => stage.status === "completed").length} / {run.stageProgress.length} 个阶段</span></div>
      ...
      <section>
            ...
            {projects.map((project) => (
              <button key={project.id} onClick={() => setSelectedId(project.id)} aria-pressed={selectedId === project.id}
                ...
                <div className="min-w-0">
                  <div className="flex flex-wrap items-center gap-2"><h4 className="font-semibold text-ink">{project.name}</h4><StatusBadge status={project.status} />{project.id === activeProject.id ? <span className="current-run-chip">当前 Run</span> : null}</div>
        ...
        <aside className="panel h-full">
          <div className="section-kicker">所选项目</div>
