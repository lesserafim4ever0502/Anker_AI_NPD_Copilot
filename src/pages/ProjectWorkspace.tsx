...
export default function ProjectWorkspace() {
  const { activeProject, activeRun: run, setActiveProject } = useProjectRun();
      ...
      <section>
        ...
        <aside className="panel h-full">
          ...
          <dl className="mt-5 grid gap-4 text-sm sm:grid-cols-2 xl:grid-cols-1">
            ...
            <div><dt className="text-xs text-slate-500">Run linkage</dt><dd className="mt-1 text-xs leading-5 text-slate-600">{selectedRun ? `${selectedRun.stageProgress.filter((stage) => stage.status === "completed").length}/${selectedRun.stageProgress.length} stages complete` : "组合占位：尚未载入完整证据快照"}</dd></div>
          </dl>
          ...
          <div className="mt-4">{selectedRun ? <Link to="/evidence-pool" onClick={() => setActiveProject(selected.id)} className="secondary-button w-full">{isActive ? "继续当前 Run" : "设为当前 Run"}<ArrowRight size={15} /></Link> : <button disabled className="secondary-button w-full cursor-not-allowed opacity-50">证据快照未载入</button>}</div>
        </aside>
        </div>
      </section>

      <section><div className="mb-3"><h3 className="section-title">当前 Run 链路</h3><p className="section-subtitle">所有页面共享同一项目、Run、决策状态与飞书数据快照</p></div><div className="grid grid-cols-2 gap-px overflow-hidden border border-slate-200 bg-slate-200 md:grid-cols-4 xl:grid-cols-7">{run.stageProgress.map((stage, index) => <div key={stage.stageId} className="bg-white p-3"><div className="text-[10px] font-semibold text-slate-400">0{index + 1}</div><div className="mt-1 text-sm font-semibold text-ink">{stage.name}</div><div className="mt-2"><StatusBadge status={stage.status} /></div></div>)}</div></section>
    </div>
  );
}
