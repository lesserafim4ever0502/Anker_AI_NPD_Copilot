...
export default function ProposalPrd() {
  ...
  return (
    <div className="space-y-6">
      ...
      <nav className="toolbar" aria-label="PRD sections" role="tablist">{tabs.map((item) => <button key={item} role="tab" aria-selected={tab === item} onClick={() => setSearchParams({ tab: item })} className={`filter-button ${tab === item ? "filter-button-active" : ""}`}>{item}</button>)}</nav>
