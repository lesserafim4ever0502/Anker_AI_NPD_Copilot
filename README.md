...
本仓库当前是 **dev-ready starter repository**，用于在正式开发前沉淀系统 PRD、设计规范、Agent 开发规范、数据采集自动化流程、数据模板和评审材料模板。当前包 **不包含真实前端开发代码**；后续由 Agent 按 `AGENT.md`、`SPEC.md`、`DESIGN.md`、`docs/DATA_INTAKE.md` 与 `docs/` 下的规范搭建 React + TypeScript + Tailwind 交互网页 Demo。
...
原则：输入证据尽量真实公开数据；标签、评分、页面映射可以由 AI 半自动生成；飞书同步和 Agent 评审在报名阶段保持 Mock。
...
报名阶段用本地 JSON 和 Mock 飞书同步展示完整流程；入围后再通过飞书 CLI 或开放平台 API 做半自动或自动化集成。
...
再实现 7 页静态可交互 Demo。优先保证叙事清晰、状态准确、数据可追溯，再做视觉 polish。
