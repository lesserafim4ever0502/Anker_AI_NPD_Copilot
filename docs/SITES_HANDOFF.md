...
|---|---|---|---|
| 1 | `ProjectWorkspace` | `projects.json`, `npdRuns.json`, `runConfig.json` | This is a multi-project NPD workspace; the main Run remains `Need Confirmation`. |
...
| 7 | `FeishuWorkflow` | `feishuWorkflow.json`, `feishuArtifacts.json`, `validationTasks.json`, `decisionLedger.json` | Feishu is the collaboration and knowledge layer, not an export button. |
...
- Warning, Pending Confirmation, Decision Ledger, Kill Criteria, and Feishu Handoff must remain visible.
...
- Show Mock Feishu handoff states without invoking a real API.
